...
# Set up the compiler. Arch is a rolling release, so this tracks whatever GCC is current
# at build time rather than a pinned version. base-devel also covers most of what vcpkg
# needs below: autoconf, automake, libtool, bison, flex, pkgconf, patch and make.
